...
const citationsController = {
    create : (req, res) => {
        ...
        let {titre, texte} = req.body;
        citationsModel.create(titre, texte)
    ...
    merge : (req, res) => {
        ...
        .then((one) =>{
            if(one[0]){
                ...
                let new_texte= req.body.texte ?? one[0].texte;
                let new_auteur_id = req.body.auteur_id ?? one[0].auteur_id;
                let new_ordre = req.body.ordre ?? one[0].ordre;
                let new_chronologie = req.body.chronologie ?? one[0].chronologie;
